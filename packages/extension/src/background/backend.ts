...
class Backend extends TypedEmitter<BackendEvents> {
    private readonly _defaultKeeWebUrl = 'https://keewebx.app/';
    ...
    async connect(): Promise<void> {
        ...
        try {
            ...
            console.error('[KeeWebX Connect] backend.connect failed:', (<Error>e).message);
    ...
    private async reconnect(): Promise<void> {
        // eslint-disable-next-line no-console
        console.log('[KeeWebX Connect] Stale session detected, reconnecting...');
        if (this._transport) {
            this._transport.removeAllListeners();
            await this._transport.disconnect();
            this._transport = undefined;
